...
class ButtonSetVolumeAction(ButtonAction):
    ...
    def onVolumeChanged(self, msg):
        if innerData(msg)['inputName'] == self.getGUIParameter(SOURCENAME_COMBO, "currentText"):
            self.volume = innerData(msg)['inputVolumeMul']
            self.logger.debug("Source {} has volume {}".format(innerData(msg)['inputName'], innerData(msg)['inputVolumeMul']))
    ...
    def onKeyUp(self):
        if self.getGUIParameter(VOLUME_MODE_COMBO, "currentIndex") == 1:
            onActionExecute(self, positionToVolume(volumeToPosition(self.volume) - 10))
        else:
            onActionExecute(self, positionToVolume(volumeToPosition(self.volume) + 10))
    #endregion